...
double alpha, beta_b;                               // Scneario[][8], [9]                % impervious to tree, spillover effect
...
void CityHEAT(double* vars, double* objs, double* constrs) {
	...
	for (int s = 0; s < Nsamples; s++) {
		...
		alpha = ScenMat[s][8]; beta_b = ScenMat[s][9];
		...
		for (int t = 1; t < (Nyears+1); t++) {
			...
			for (int r = 0; r < Nregions; r++) {
				...
				while ((adjacent[r][j] >= 0) && (j < 5)) {
					temp_miti_spill[r][t] = temp_miti_spill[r][t] + beta_b * District[adjacent[r][j]][0] / District[r][0] * temp_miti[adjacent[r][j]][t];  // spillover effect
					j = j + 1;
				}
