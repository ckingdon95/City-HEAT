...
//double HWD[Nregions][Nyears] = { 0 };
//double HWD0[Nregions][Nyears] = { 0 };
...
void CityHEAT(double* vars, double* objs, double* constrs) {
	...
	for (int s = 0; s < Nsamples; s++) {
		...
		for (int r = 0; r < Nregions; r++) {
			...
			/*
			for (int j = 0; j < Nsummer; j++) {
				temp_r_0[j] = temp[ClimModel][j][0] + District[r][6];   //  District[r][6] =  dT[r];
			}

			// which heaw wave function to use 
			if (HWdef == 1) {										    // ScenMat[s][2] index for heat wave definition
				HWD[r][0] = hw_count1(temp_r_0, 35, 3);		            //Huang 2010, Daily maximum temperature > 35 degree C for at least 3 consecutive days
			}
			else if (HWdef == 2) {
				HWD[r][0] = hw_count1(temp_r_0, 35.0, 2);		        // AB 2011 Daily maximum temperature > 95th percentile for >= 2 consecutive days
			}
			else {
				HWD[r][0] = hw_count2(temp_r_0, 36.1, 32.2, 3);       // Peng  Double threshold T1 = 97.5% and T2 = 81%
		...
		for (int t = 1; t < (Nyears+1); t++) {
			...
			for (int r = 0; r < Nregions; r++) {
				...
				/*
				if (HWD[r][(t + 4)] > HWD0[r][(t + 4)]){
					HWD[r][(t + 4)] = HWD0[r][(t + 4)];  // to avoid numeric errors
				}
                                */
